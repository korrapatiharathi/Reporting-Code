...
for (i in 1:length(inputFolders)) {
  ...
  # Loop through the files to perform appropriate analysis
   for (i in 1:length(files)) {
    ...
    df3$path <- as.factor(toupper(df3$path))
    maxDisk <- as.data.frame(df3 %>% group_by(date, path) %>%
                                 summarise(max = max(used_percent)))
    maxDisk <- (as.data.frame(spread(maxDisk, key = path, value = max)))
    ...
    write.xlsx(maxDisk, file = paste0(diskFolder, filename,"", "_Disk_Output",".xlsx",sep = ''), sheetName="Max Disk")
    }
    ...
    #maxMem <- (as.data.frame(spread(maxMem, key=date, value = max)))
    # Create a new folder for outputs
    memFolder <- paste0(WD, "/OUTPUTS/Memory_Output/")
    colnames(maxMem)[colnames(maxMem) == "max"] <- filename
    ...
    # Check if the file type is cpu or not; if yes, calculate maximum cpu load per day
      ...
      colnames(maxCpu)[colnames(maxCpu) == "max"] <- filename
      #maxCpu <- (as.data.frame(spread(maxCpu, key=date, value = max)))
      # Create a new folder for outputs
      ...
      # Write the outputs to excel file
      write.xlsx(maxCpu, file = paste0(cpuFolder, filename, "_CPU_Output",".xlsx",sep = ''), sheetName="Max CPU")  
